...
| New `embed-test` page | Day 2: added the [Block Collection `embed` block](https://www.aem.live/developer/block-collection/embed) (Adobe-vetted, used mostly as-is) — lazy-loads a YouTube video or X/Twitter post from a plain link, with an `aria-label` added to the play-button placeholder (source had none) and `x.com` added alongside `twitter.com` for domain matching. Verified locally: video plays inline, tweet renders via Twitter's widget script; unrelated YouTube ad-conversion CORS console noise confirmed not caused by this block. | `embed` | Code in PR, CMS authoring pending | (this PR) |
